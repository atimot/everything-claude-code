...
-- 対応するクエリ: WHERE status = 'pending' AND created_at > '2024-01-01'
```
...
-- より小さいインデックス、アクティブなユーザーのみ含む
```

**RLSポリシー（最適化済み）：**
```sql
CREATE POLICY policy ON orders
  USING ((SELECT auth.uid()) = user_id);  -- SELECTでラップすること！
```
...
- スキル: `clickhouse-io` - ClickHouseアナリティクスパターン
- スキル: `backend-patterns` - APIおよびバックエンドパターン
